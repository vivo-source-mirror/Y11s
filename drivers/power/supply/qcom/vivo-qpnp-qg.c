...
#include "qg-defs.h"
#include "vivo-fuel_summary.h"  //vivo add
			       ...
			       union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_MIN:
		...
		pval->intval = chip->last_cutoff_volt_mv * 1000;
		break;
...
static void qg_monitor_work(struct work_struct *work)
{
	...
	int rc = 0, soc_now, vbatt_now, ibatt_now, batt_temp_now, ocv_now;
	...
	pr_err("charge_status=%d,soc=%d,vbatt=%d,ibatt=%d,batt_temp=%d,ocv=%d,battid=%d,ui_soc=%d,learned_cap=%d,nominal_cap=%d\n",
					chip->charge_status, soc_now, vbatt_now, ibatt_now, batt_temp_now, ocv_now, chip->cms_batt_id, ui_soc.intval,
					learned_cap/1000, nominal_cap/1000);
	...
	fuelsummary_collect_value(ID_SOH, chip->soh);
#endif
