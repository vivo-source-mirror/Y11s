...
static int force_5v_hvdcp(struct smb_charger *chg);
...
void boost_back_work_variable_init(struct smb_charger *chg);
#define CHARGER_BOOST_BACK_WORK_PEROID_MS	3000
/*end*/
...
static int vivo_get_prop_batt_status(struct smb_charger *chg)
{
	int rc;
	u8 stat;
				...
				union power_supply_propval *val)
{
	...
	/* vivo add start */
	if(chg->usb_present) {
		if (atomic_read(&chg->chg_boost_back_flag)
			&& chg->last_charge_status == POWER_SUPPLY_STATUS_CHARGING) {
			val->intval = POWER_SUPPLY_STATUS_CHARGING;
			smblib_dbg(chg, PR_MISC, "boost back workround is running, usb last charging status\n");
		}
		chg->last_charge_status = val->intval;
	}
	/* vivo add end */
...
void smblib_usb_plugin_hard_reset_locked(struct smb_charger *chg)
{
	...
	if (vbus_rising) {
		...
		boost_back_work_variable_init(chg);
	}
...
irqreturn_t usb_source_change_irq_handler(int irq, void *data)
{
	...
	if (is_present) {
		...
		if (chg->real_charger_type == POWER_SUPPLY_TYPE_USB) {
			...
			/* add for charger boost back */
			if(!chg->first_dcp_charger_detect_done) {
				chg->first_dcp_charger_detect_done = true;
				cancel_delayed_work(&chg->charger_boost_back_monitor_work);
				schedule_delayed_work(&chg->charger_boost_back_monitor_work,
						msecs_to_jiffies(CHARGER_BOOST_BACK_WORK_PEROID_MS));
			}
		}
...
irqreturn_t typec_attach_detach_irq_handler(int irq, void *data)
{
	...
	if (attached) {
		...
		if (0 && chg->typec_port && !chg->pr_swap_in_progress)/*vivo modify*/
...
static void smblib_bb_removal_work(struct work_struct *work)
{
	...
	/* vivo add start */
	atomic_set(&chg->chg_boost_back_flag, 0);
	/* vivo add end */
}
...
irqreturn_t switcher_power_ok_irq_handler(int irq, void *data)
{
	...
	if (is_storming(&irq_data->storm_data)) {
		...
		if (!is_client_vote_enabled(chg->usb_icl_votable,
			...
			/* vivo add start */
			if (atomic_read(&chg->chg_boost_back_flag)) {
				smblib_err(chg, "private boost back workround is running!\n");
				return IRQ_HANDLED;
			}
			atomic_set(&chg->chg_boost_back_flag, 1);
			/* vivo add end */
			smblib_err(chg,
...
int smblib_factory_set_chg_vbus_voltage(struct smb_charger *chg, int val)
{
	if (!chg->usb_present) {
		smblib_dbg(chg, PR_MISC, "usb can't plugin,not set chg vbus voltage!\n");
		return 0;
	...
	smblib_dbg(chg, PR_MISC, "factory test try to chg_vbus_voltage:%dv.\n", val);

	switch (val) {
	case HVDCP_TYPE_5V:
		chg->cc_force_5v_flag |= CCF5_FLAG_FACTORY_TEST;
		force_5v_hvdcp(chg);
		break;
	case HVDCP_TYPE_9V:
		chg->cc_force_5v_flag &= (~CCF5_FLAG_FACTORY_TEST);
...
static int smblib_charge_condition_force_5v(struct smb_charger *chg, int chg_status, int batt_soc, int chg_volt_v)
{
	...
	if (batt_soc >= chg->cc_force_5v_soc_limit ||
		...
		}else if(chg->cc_force_5v_flag & CCF5_FLAG_FACTORY_TEST){
			smblib_dbg(chg, PR_MISC, "factory test control,cc input adjust is not need\n");
...
void boost_back_work_variable_init(struct smb_charger *chg)
{
	cancel_delayed_work(&chg->charger_boost_back_monitor_work);
	atomic_set(&chg->chg_boost_back_flag, 0);
	chg->first_dcp_charger_detect_done = false;
	chg->low_input_current_counter = 0;
	chg->last_charge_status = POWER_SUPPLY_STATUS_UNKNOWN;
}
/* charger boost back workround */
static void smblib_boost_back_monitor_work(struct work_struct *work)
{
	struct smb_charger *chg = container_of(work, struct smb_charger,
								charger_boost_back_monitor_work.work);
	union power_supply_propval usb_input_current = {0,};
	union power_supply_propval bat_current = {0,};
	union power_supply_propval batt_soc = {0,};
	int rc = 0, usb_icl = 0;
	u8 stat = 0;
	int usb_is_suspend = 0;
	int batt_status;

	if(!chg->usb_present){
		smblib_dbg(chg, PR_MISC, "usb is not present, exit boost back work around!\n");
		goto abort2;
	}
	rc = smblib_read(chg, POWER_PATH_STATUS_REG, &stat);
	if (rc < 0) {
		smblib_dbg(chg, PR_MISC, "Couldn't read POWER_PATH_STATUS rc=%d\n", rc);
		goto abort1;
	}
	if (stat & USE_DCIN_BIT) {
		smblib_dbg(chg, PR_MISC, "use dcin, exit boost back work around\n");
		goto abort2;
	}
	/* cancel boost back work when battery full */
	smblib_get_prop_batt_capacity(chg, &batt_soc);
	if((chg->recharge_flag == true || chg->charge_full == true) && batt_soc.intval == 100)
	{
		smblib_dbg(chg, PR_MISC, "battery is full,exit boost back work around \n");
		goto abort2;
	}
	/* skip suspending input if in weak charger mode */
	if(chg->chg_abnormal_flag & CHG_WEAK_CHARGER_ERROR) {
		smblib_dbg(chg, PR_MISC, " skip suspending input in weak charger mode\n");
		goto abort1;
	}
	/* skip suspending input if its already suspended by some other voter */
	usb_icl = get_effective_result(chg->usb_icl_votable);
	if ((stat & USE_USBIN_BIT) && usb_icl >= 0 && usb_icl <= USBIN_25MA) {
		smblib_dbg(chg, PR_MISC, "stat=%d,usb_icl=%d, usb_present=%d,usb_is_suspend=%d,skip work workround\n",
			stat, usb_icl, chg->usb_present, usb_is_suspend);
		goto abort1;
	}
	/* skip suspending input if its already suspended */
	rc = smblib_get_usb_suspend(chg, &usb_is_suspend);
	if (rc < 0 || (chg->usb_present && usb_is_suspend)) {
		smblib_dbg(chg, PR_MISC, "usb_present=%d,usb_is_suspend=%d,rc=%d\n",
			chg->usb_present, usb_is_suspend, rc);
		goto abort1;
	}

	/* skip suspending input if its already in direct charing mode */
	if(chg->dchg_status == DIRECT_CHARGER_PREPARE
		||chg->dchg_status == DIRECT_CHARGER_IS_CHARGERING) {
		smblib_dbg(chg, PR_MISC, "dchg_status=%d,skip work workround\n", chg->dchg_status);
		goto abort1;
	}
	if(!chg->usb_present){
		smblib_dbg(chg, PR_MISC, "usb is not present, exit boost back work around \n");
	...
	/* get usb input current now */
	rc = smblib_get_prop_usb_current_now(chg, &usb_input_current);
	if (rc < 0) {
		smblib_dbg(chg, PR_MISC, "get input current fail rc=%d\n", rc);
		goto abort1;
	}
	/* get battery current now */
	rc = smblib_get_batt_current_now(chg, &bat_current);
	if (rc < 0) {
		smblib_dbg(chg, PR_MISC, "get bat current fail rc=%d\n", rc);
		goto abort1;
	}
	batt_status = vivo_get_prop_batt_status(chg);
	/* input current threshold is 50mA,battery current threshold is 30mA */
	if(batt_status == POWER_SUPPLY_STATUS_CHARGING && usb_icl >= USBIN_100MA
		&& usb_input_current.intval < 75000 && bat_current.intval > 20000)
		chg->low_input_current_counter++;
	else
		chg->low_input_current_counter = 0;
	smblib_dbg(chg, PR_MISC, "batt_status=%d,usb_icl=%d,usb_input_current=%d,bat_current=%d,low_input_current_counter=%d, chg_boost_back_flag=%d, usb_present=%d\n",
		batt_status, usb_icl, usb_input_current.intval, bat_current.intval, chg->low_input_current_counter, atomic_read(&chg->chg_boost_back_flag), chg->usb_present);

	/* if reverse boost detected, voting 0mA to suspend input */
	if(chg->low_input_current_counter >= 3 && chg->usb_present) {
		chg->low_input_current_counter = 0;
		if (atomic_read(&chg->chg_boost_back_flag)) {
			smblib_dbg(chg, PR_MISC, "switch power ok boost back work runing!\n");
			goto abort1;
		}
		atomic_set(&chg->chg_boost_back_flag, 1);
		smblib_dbg(chg, PR_MISC, "Reverse boost detected: voting 0mA to suspend input, boost_counter=%d\n",
										chg->chg_boost_back_counter);
		vote(chg->usb_icl_votable, BOOST_BACK_VOTER, true, 0);
		vote(chg->awake_votable, BOOST_BACK_VOTER, true, 0);
		 /* Remove the boost-back vote after a delay, to avoid
		 * permanently suspending the input if the boost-back
		...
		schedule_delayed_work(&chg->bb_removal_work,
			msecs_to_jiffies(BOOST_BACK_UNVOTE_DELAY_MS));
		chg->chg_abnormal_flag |= CHG_BOOST_BACK_POLL_ERROR;
		/* boost back counter*/
		chg->chg_boost_back_counter++;
		if(chg->chg_boost_back_counter > 5000)
			chg->chg_boost_back_counter = 5000;
	}
	schedule_delayed_work(&chg->charger_boost_back_monitor_work,
				msecs_to_jiffies(CHARGER_BOOST_BACK_WORK_PEROID_MS));
	return;
...
static void smblib_usb_conn_pro_monitor_work(struct work_struct *work)
{
	...
	if (((cms_health_now.intval & 0x4000) && (last_cms_health & 0x4000)) || chg->usb_conn_pro_flag == 1) {	//need check this 0x4000 in cms
		chg->cc_force_5v_usb_conn_protect = 1;//cms == 0x4000, force 9v to 5v
		work_period_ms = 300;
	}
...
int smblib_init(struct smb_charger *chg)
{
	...
	INIT_DELAYED_WORK(&chg->charger_boost_back_monitor_work, smblib_boost_back_monitor_work);
...
int smblib_deinit(struct smb_charger *chg)
{
	...
	case PARALLEL_MASTER:
		...
		cancel_delayed_work_sync(&chg->role_reversal_check);
		cancel_delayed_work_sync(&chg->pr_swap_detach_work);
		cancel_delayed_work_sync(&chg->otg_oc_work);//vivo add
		cancel_delayed_work_sync(&chg->force_9v_hvdcp_work);//vivo add
		...
		cancel_delayed_work_sync(&chg->charger_boost_back_monitor_work);
		/* vivo end */
