...
static int smb5_parse_dt_misc(struct smb5 *chip, struct device_node *node)
{
	...
	chg->usb_conn_pro_enable_flag = 0;
	/* vivo add */
	chg->pd1965_flag = of_property_read_bool(node, "qcom,pd1965-flag");
	chg->pd2034_flag = of_property_read_bool(node, "qcom,pd2034-flag");
	chg->pd2042_flag = of_property_read_bool(node, "qcom,pd2042-flag");
	...
	else if(chg->pd2034_flag){
		pr_info("czt 2034 Enter\n");
		board_version = get_bbk_board_version();
		if (!board_version) {
			chg->enable_slave_charger = true;
	...
	} else if (chg->pd2042_flag) {
		pr_err("czt 2042 Enter\n");
		board_version = get_bbk_board_version();
		if ((!strncmp(board_version, "CFD", 3))) {
			chg->usb_conn_pro_enable_flag = 1;
			pr_err("set usb_conn_pro_enable_flag done ...\n");
		}
	} else {
	...
	//if ((!chg->pd1965_flag) || (chg->pd1965_flag && chg->enable_slave_charger && (!chg->hvdcp_disable))) {
	...
	//}
	if ((!chg->pd2034_flag) || (chg->pd2034_flag && chg->enable_slave_charger && (!chg->hvdcp_disable))) {
		chip->dt.hvdcp_disable = of_property_read_bool(node,
							"qcom,hvdcp-disable");
		chg->hvdcp_disable = chip->dt.hvdcp_disable;
		//fuelsummary_of_property_put("qcom,hvdcp-disable", PARAMS_TYPE_BOOL, &(chg->hvdcp_disable));
	...
	if(chg->pd2034_flag && !chg->enable_slave_charger){
		pr_err("5v charger, donot use usbsel-gpio.\n");
		chg->usbsel_gpio = -1;
	}
	...
	if(chg->pd2034_flag && !chg->enable_slave_charger && chg->hvdcp_disable){
		pr_err("5v 2acharger, current spicail change\n");
		chg->current_spec_min = 1500;
		chg->current_spec_max = 2200;
	}
	...
	if (rc < 0)
		...
		if (chg->supported_adapter >= SUPPORTED_ADAPTER_MAX)
			chg->supported_adapter = SUPPORTED_ADAPTER_DEFAULT;
		if(chg->pd2034_flag && !chg->enable_slave_charger && chg->hvdcp_disable){
			pr_err("5v 2acharger, type spicail change\n");
			chg->supported_adapter = SUPPORTED_ADAPTER_DEFAULT;
		}
	...
	if (!chg->usb_conn_pro_enable  &&  chg->usb_conn_pro_enable_flag)
		chg->usb_conn_pro_enable = 1;
		...
		const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_FACTORY_10W_CHARGE_TEST:
		pr_err("### factory test state = %d ###\n", val->intval);
		if (val->intval)
				rc = smblib_factory_set_chg_vbus_voltage(chg, HVDCP_TYPE_5V);
			else
				rc = smblib_factory_set_chg_vbus_voltage(chg, HVDCP_TYPE_9V);
	case POWER_SUPPLY_PROP_FLASHLED_STATE:
