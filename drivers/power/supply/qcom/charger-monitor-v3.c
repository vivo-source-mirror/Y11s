...
enum charge_test_item {
	cableR,				/* cable	resistance, min value, max value */
	M_ibus,				/* master ibus, min ibus, max ibus */
	S_ibus,				/* slave ibus, min ibus, max ibus */
	BATT_THERM,			/* batt therm ntc, min value, max value */
	BOARD_THERM,		/* board therm ntc, min value, max value */
	SUB_BOARD_THERM,	/* sub charger ntc, min value, max value */
	USB_CON_THERM,		/* usb con ntc, min value, max value */
	USB_CON_1_THERM,	/* usb con 1 ntc, double usb NTC, min value, max value */
	BAT_BTB_THERM,		/* bat btb master ntc, min value, max value */
	BAT_BTB_THERM_M,	/* bat btb sub ntc, min value, max value */
	BAT_BOARD_THERM,	/* bat board ntc, min value, max value */
	WLC_COIL_THERM,		/* wireless charge coil ntc, min value, max value */
	Unkown_test_item
};
				...
				size_t count, loff_t *ppos)
{
	...
	cms_ensure_psy_available(chip);
	if(!chip->batt_psy || (!chip->mcu_psy && (supported_adapter.intval == 2 || supported_adapter.intval == 4 || supported_adapter.intval == 5))) {
		cms_print(chip,PR_ERROR,"bat_psy or mcu_psy not registed\n");
		return ret;
	}
...
struct device_node *cms_find_support_board_node(struct cms *chip)
{
	...
	char char_board_version;
	int rc = 0;

	board_version= get_bbk_board_version();
	for_each_child_of_node(chip->dev->of_node, node) {
		chip->pd2034_flag = of_property_read_bool(node, "qcom,pd2034-flag");
		if(chip->pd2034_flag){
			if ((!strncmp(board_version, "AAA", 3)) || (!strncmp(board_version, "ABB", 3)) || (!strncmp(board_version, "ABE", 3)) \
				...
				//dual charger
				char_board_version = '1';
			}else{
				char_board_version = '0';
			}
			rc = of_property_read_string(node, "support-board-version",
									&support_board_version);
			if (!rc) {
				if (char_board_version == support_board_version[7]) {
					match_node = node;
...
static int cms_parse_dt(struct cms *chip)
{
	...
	/*********************************************/
	chip->pd2042_flag = of_property_read_bool(node, "qcom,pd2042-flag");
	chip->usb_connecter_protect_enable_flag = 0;
	...
	board_version = get_bbk_board_version();
	 ...
	 if (chip->pd2042_flag) {
			...
			if ((!strncmp(board_version, "CFD", 3))) {
				chip->usb_connecter_protect_enable_flag = 1;
				pr_err(" set usb_connecter_protect_enable_flag ... done\n");
	...
	if (!chip->usb_connecter_protect_enable && chip->usb_connecter_protect_enable_flag)
		chip->usb_connecter_protect_enable = 1;
	...
	if(chip->charge_test_param_rc) {
		...
		if (chip->usb_connecter_protect_enable_flag) {
			chip->charge_test_param_data[USB_CON_THERM].enable_switch = 1;
			pr_err("charge_test_param_data[USB_CON_THERM].enable_switch...1 !\n");
		}
	}
