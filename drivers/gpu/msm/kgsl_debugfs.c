...
static int process_mem_show_quick(struct seq_file *s, void *v)
{
	...
	while (1) {
		...
		if (kgsl_mem_entry_get(entry)) {
			...
			if (m->size <= 0 || atomic_long_read(&m->mapsize) < 0 || atomic_long_read(&m->mapsize) > m->size) {
			...
			if (usermem_type == KGSL_MEM_ENTRY_KERNEL) {
				if (m->useraddr) {
					gl_counted += atomic_long_read(&m->mapsize);
					gl_uncounted += (m->size - atomic_long_read(&m->mapsize));
				} else {
					gl_uncounted += m->size;
				}
