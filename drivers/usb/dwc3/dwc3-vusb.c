...
struct dwc3_vusb {
	...
	struct delayed_work host_detect_work;
					 ...
					 const char *buf, size_t size)
{
	...
	if (!dwc3_vusb->mode) {
		...
		if (is_enable) {
			...
			queue_delayed_work(dwc3_vusb->workq, &dwc3_vusb->host_detect_work, 0);
			...
			cancel_delayed_work(&dwc3_vusb->host_detect_work);
...
static void dwc3_vusb_host_detect_work(struct work_struct *work)
{
	struct dwc3_vusb *dwc3_vusb = container_of(work,
						   struct dwc3_vusb,
						   host_detect_work.work);
	int i, ret, voltage = 0;
	union power_supply_propval pval = {0,};
	bool id;
	...
	queue_delayed_work(dwc3_vusb->workq, &dwc3_vusb->host_detect_work,
...
static int dwc3_vusb_probe(struct platform_device *pdev)
{
	...
	if (dwc3_vusb->mode) {
		...
		INIT_DELAYED_WORK(&dwc3_vusb->host_detect_work, dwc3_vusb_host_detect_work);
...
static int dwc3_vusb_remove(struct platform_device *pdev)
{
	...
	if (dwc3_vusb->mode) {
		cancel_delayed_work_sync(&dwc3_vusb->host_detect_work);
