...
static void qusb_phy_get_tune2_param(struct qusb_phy *qphy)
{
	...
	tune2_val_correction = qphy->tune2_val;
	if (qphy->phy.otg_host_state)
		tune2_val_correction += qphy->tune2_efuse_correction_host;
	else
		tune2_val_correction += qphy->tune2_efuse_correction;

	if (tune2_val_correction < 0)
		tune2_val_correction = 0;
	if (tune2_val_correction > bit_mask)
		tune2_val_correction = bit_mask;

	pr_debug("(mode: %d)efuse=%d, correction=%d, value=%d\n", qphy->phy.otg_host_state, qphy->tune2_val,
			qphy->phy.otg_host_state ? qphy->tune2_efuse_correction_host : qphy->tune2_efuse_correction, tune2_val_correction);

	reg_val = readb_relaxed(qphy->base + QUSB2PHY_PORT_TUNE2);
	if (qphy->tune2_val) {
		reg_val  &= 0x0f;
		reg_val |= (tune2_val_correction << 4);
	}
...
static int qusb_phy_init(struct usb_phy *phy)
{
	...
	pr_debug("%s():userspecified modparams TUNEX val:0x%x %x %x %x %x\n",
				__func__, qphy->tune1, qphy->tune2, qphy->tune3, qphy->tune4, qphy->tune5);
	if (qphy->tune1) {
	...
	pr_debug("%s: (mode: %d)TUNE1:%08x TUNE2:%08x TUNE3:%08x TUNE4:%08x TUNE5:%08x\n",
		__func__, qphy->phy.otg_host_state, readl_relaxed(qphy->base + QUSB2PHY_PORT_TUNE1),
		readl_relaxed(qphy->base + QUSB2PHY_PORT_TUNE2), readl_relaxed(qphy->base + QUSB2PHY_PORT_TUNE3),
		readl_relaxed(qphy->base + QUSB2PHY_PORT_TUNE4), readl_relaxed(qphy->base + QUSB2PHY_PORT_TUNE5));
