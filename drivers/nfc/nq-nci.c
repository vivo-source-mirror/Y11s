...
struct nqx_dev {
	...
	struct  pinctrl_state *pins_en_active;
	struct  pinctrl_state *pins_en_suspend;
...
// vivo add nzf for shutdown charging card simulation begin
extern unsigned int power_off_charging_mode;
// vivo add nzf for shutdown charging card simulation end
...
static int sn100_ese_pwr(struct nqx_dev *nqx_dev, unsigned long int arg)
{
	...
	if (arg == ESE_POWER_ON) {
		...
		if (!nqx_dev->nfc_ven_enabled) {
			...
			if (pinctrl_select_state(nqx_dev->pinctrl_gpios, nqx_dev->pins_en_active) < 0) {
				dev_err(&nqx_dev->client->dev,
					"nfc failed to selected \n");
			}
		...
		if (!nqx_dev->nfc_ven_enabled) {
			...
			if (pinctrl_select_state(nqx_dev->pinctrl_gpios, nqx_dev->pins_en_suspend) < 0) {
				dev_err(&nqx_dev->client->dev,
					"nfc failed to selected \n");
			}
			/* hardware dependent delay */
...
int nfc_ioctl_power_states(struct file *filp, unsigned long arg)
{
	...
	printk("nfc nfc_ioctl_power_states :%ld", arg);
...
static int nfcc_hw_check(struct i2c_client *client, struct nqx_dev *nqx_dev)
{
	...
	printk("nfc enter nfcc_hw_check");
...
static int nfc_pinctrl_init(struct nqx_dev *nqx_dev)
{
	int ret = 0;
	nqx_dev->pinctrl_gpios = devm_pinctrl_get(&nqx_dev->client->dev);
	if (IS_ERR_OR_NULL(nqx_dev->pinctrl_gpios)) {
		dev_err(&nqx_dev->client->dev,
				"nfc pinctrl_gpios is not exist\n");
		goto err_pinctrl_get;
	}
	nqx_dev->pins_en_active = pinctrl_lookup_state(nqx_dev->pinctrl_gpios, "nfc_active");
	if (IS_ERR_OR_NULL(nqx_dev->pins_en_active)) {
		dev_err(&nqx_dev->client->dev,
				"nfc pins_en_active is not exist\n");
		goto err_pinctrl_lookup;
	}
	nqx_dev->pins_en_suspend = pinctrl_lookup_state(nqx_dev->pinctrl_gpios, "nfc_suspend");
	if (IS_ERR_OR_NULL(nqx_dev->pins_en_suspend)) {
		dev_err(&nqx_dev->client->dev,
				"nfc pins_en_suspend is not exist\n");
		goto err_pinctrl_lookup;
	}
	if (pinctrl_select_state(nqx_dev->pinctrl_gpios, nqx_dev->pins_en_suspend) < 0) {
		dev_err(&nqx_dev->client->dev,
	...
	return 0;
err_pinctrl_lookup:
	devm_pinctrl_put(nqx_dev->pinctrl_gpios);
err_pinctrl_get:
	nqx_dev->pinctrl_gpios = NULL;
	return ret;
}
			...
			const struct i2c_device_id *id)
{
	...
	if (nfc_support == 1) {
		if (boardversion_num == 0) {
			...
			for (i = 0; i < boardversion_num; i++) {
				...
				if (strncmp(boardversions[i], board_version, boardversion_mask) == 0) {
					dev_err(&client->dev, "nfc The machine has NFC HW\n");
					nfc_result = 1;
					break;
				}
			}
			if (i == boardversion_num) {
				dev_err(&client->dev, "nfc The machine hasn't NFC HW\n");
				nfc_result = 0;
			}
		}
	} else {
	...
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
		dev_err(&client->dev, "%s: nfc need I2C_FUNC_I2C\n", __func__);
		r = -ENODEV;
		goto err_free_data;
	}
    ...
    // modify by nzf for shutdown cardEmulation begin
    r = nfc_pinctrl_init(nqx_dev);
	if (IS_ERR_OR_NULL(nqx_dev->pinctrl_gpios)) {
		devm_pinctrl_put(nqx_dev->pinctrl_gpios);
		nqx_dev->pinctrl_gpios = NULL;
		goto err_free_dev;
	}
    // modify by nzf for shutdown cardEmulation end
	...
	if (gpio_is_valid(platform_data->en_gpio)) {
		...
		if (power_off_charging_mode == 1) {
            ...
            if (pinctrl_select_state(nqx_dev->pinctrl_gpios, nqx_dev->pins_en_active) < 0) {
                dev_err(&nqx_dev->client->dev, "nfc failed to selected \n");
            }
        }
		else {
