...
#define EXPECTED_MODEM_CRASH	"This crash is expected!!!"
...
/* vivo leichenji add for identify permissive subsys crash */
static void vivo_identify_permissive_subsys_crash_by_reason(struct subsys_device *subsys,
					const char *subsys_name, char *crash_reason)
{
	if (is_atboot && strnstr(subsys_name, "modem", sizeof(subsys_name))
		&& strnstr(crash_reason, EXPECTED_MODEM_CRASH, strlen(crash_reason))) {
		subsys_mark_permissive_crash(subsys, true);
	}
}
/* vivo leichenji add for identify permissive subsys crash end */
...
static void log_failure_reason(const struct pil_tz_data *d)
{
	...
	/* vivo identify permissive subsys crash */
	vivo_identify_permissive_subsys_crash_by_reason(d->subsys, name, reason);
	/* vivo identify permissive subsys crash end */
