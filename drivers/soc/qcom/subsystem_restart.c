...
#include <linux/idr.h>
//vivo mrq add for subsys begin
#include <linux/debugfs.h>
//vivo mrq add for subsys end
...
struct subsys_device {
	...
	/* vivo add for permissive subsys crash */
	bool permissive_crash;
};
...
/* vivo leichenji add to set permissive subsys crash */
void subsys_mark_permissive_crash(struct subsys_device *subsys, bool permissive)
{
	subsys->permissive_crash = 	permissive;
}
/* vivo leichenji add to set permissive subsys crash end */
...
{
	const char *name;
	int temp_restart_level = dev->restart_level;
	...
	case RESET_SUBSYS_COUPLED:
		...
		break;
	case RESET_SOC:
		/* vivo change for permissive subsys crash */
		if (!dev->permissive_crash) {
			__pm_stay_awake(&dev->ssr_wlock);
			schedule_work(&dev->device_restart_work);
			return 0;
		} else {
			pr_warn("%s this is permissive crash, so ignore restart_level to restart subsys %s\n",
					__func__, name);
			__subsystem_restart_dev(dev);
			dev->permissive_crash = false;
			break;
		}
		/* vivo add for permissive subsys crash end */
	default:
