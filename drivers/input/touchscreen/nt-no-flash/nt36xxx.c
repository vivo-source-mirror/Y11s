...
static int fb_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
	...
	NVT_LOG("enter nvt fb_notifier_callback ");
	if (evdata && evdata->data && event == VTS_EARLY_EVENT_BLANK) {
		blank = evdata->data;
		if (*blank == VTS_BLANK_POWERDOWN) {
			NVT_LOG("event=%lu, *blank=%d\n", event, *blank);
			nvt_ts_suspend(&ts->client->dev);
		}
	} else if (evdata && evdata->data && event == VTS_EVENT_BLANK) {
		blank = evdata->data;
		if (*blank == VTS_BLANK_UNBLANK) {
			NVT_LOG("event=%lu, *blank=%d\n", event, *blank);
...
static int parse_panel(struct device *dev)
{
	int i;
	int count;
...
static void nvt_ts_work_func(struct work_struct *work)
	...
	//[20190815,jx]Add
#if WAKEUP_GESTURE
		if (bTouchIsAwake == 0) {
			pm_wakeup_event(&ts_spi->input_dev->dev, 5000);
		}
#endif
		...
		ret = CTP_SPI_READ(ts_spi->client, point_data, POINT_DATA_LEN + 1);
		if (ret < 0) {
	...
	*/
	   ...
	   if (nvt_wdt_fw_recovery(point_data)) {
		   mutex_lock(&ts_spi->lock);	//[20200703,jx]Add
		   NVT_ERR("Recover for fw reset, %02X\n", point_data[1]);
		   nvt_update_firmware(BOOT_UPDATE_FIRMWARE_NAME, 1 ,0);
...
return:
	irq execute status.
*******************************************************/
static irqreturn_t nvt_ts_irq_handler(int irq, void *data)
{
...
if(0) {
	...
	parse_panel(&client->dev);
	ts_spi->fb_notif.notifier_call = fb_notifier_callback;
	if(ts_spi->active_panel_v2){
		ret = drm_panel_notifier_register(ts_spi->active_panel_v2 ,&ts_spi->fb_notif);
		if(ret) {
...
if(ts_spi->active_panel_v2)
	drm_panel_notifier_unregister(ts_spi->active_panel_v2 ,&ts_spi->fb_notif);
