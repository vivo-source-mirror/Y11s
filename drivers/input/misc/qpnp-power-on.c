...
struct qpnp_pon {
	...
	/*Begin add kpdpwr work hankecai 2017-08-28*/
	struct delayed_work kpdpwr_work;
	/*End add kpdpwr work hankecai 2017-08-28*/
	u16			base;
	u8			subtype;
	u8			pon_ver;
...
static u32 kpdpwr_work_timeout = 8;
static bool kpdpwr_work_state;
...
static void kpdpwr_work_func(struct work_struct *work)
{
	...
	char *work_state[2] = {"kpdpwr_work_state=1", NULL};
	struct qpnp_pon *pon =
	...
	if (key_status == (QPNP_PON_KPDPWR_N_SET | QPNP_PON_RESIN_N_SET)) {
		...
		kpdpwr_work_state = 1;
		pr_err("%s\n", work_state[0]);
		kobject_uevent_env(&pon->dev->kobj, KOBJ_CHANGE, work_state);
	}
}

static ssize_t qpnp_pon_kpdpwr_work_state_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct qpnp_pon *pon = dev_get_drvdata(dev);
	struct qpnp_pon_config *cfg = NULL;
	...
	temp_val = !!kpdpwr_work_state;
	return snprintf(buf, sizeof(temp_val), "%d\n",
			temp_val);
}
static DEVICE_ATTR(kpdpwr_work_state, 0444,
		qpnp_pon_kpdpwr_work_state_show, NULL);

static ssize_t kpdpwr_work_timeout_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct qpnp_pon *pon = dev_get_drvdata(dev);
	struct qpnp_pon_config *cfg = NULL;
	...
	return snprintf(buf, sizeof(kpdpwr_work_timeout), "%d\n",
			kpdpwr_work_timeout);
}

static ssize_t kpdpwr_work_timeout_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t size)
{
	struct qpnp_pon *pon = dev_get_drvdata(dev);
	...
	if (5 <= value && 10 >= value)
		kpdpwr_work_timeout = value;

	return size;
}
static DEVICE_ATTR(kpdpwr_work_timeout, 0664,
		kpdpwr_work_timeout_show,
		kpdpwr_work_timeout_store);
...
int qpnp_pon_wd_cfg_timer(u8 s1_timer, u8 s2_timer)
{
	...
	rc = qpnp_pon_masked_write(pon, addr,
				0x0F, PON_POWER_OFF_HARD_RESET); /*hard reset*/
...
void vivo_pmic_wdt_prepare_and_start(void)
{
	...
	/*?¦Ì?¡Â
	 *g_force_offline_init¡À¨º??¨º?¡¤?¨º1wdt_monitor¨¦¨²D¡ì¡ê??¨¦??¨¨???:
	 *1?¡é¡ä¨°?a¨¢?vivolog¡ê?wdt2?¨¦¨²D¡ì
	 *2?¡é¦Ì¡Â¨º??¡ê¨º?(eng?¡ébsptmode)¡ê?wdt2?¨¦¨²D¡ì
	 *3?¡é????force_offline¡À¨º??¡À???¨¦?¨¢?¡ê?wdt2?¨¦¨²D¡ì
	 *4?¡é?¨²force_offline??¨®D¡À???¨¦?¦Ì??¨¦????¡ê??¨¦¨°?¡À?wdt_test¡À¨º??????¨¦¨²D¡ì
	 */
	...
	if (power_off_charging_mode) {
		...
		//vivo_pmic_wdt_start_kernel(300);	/*300 seconds*/
...
/*Begin hankecai add kpdpwr state 2017-08-08*/
static irqreturn_t qpnp_kpdpwr_thread_fn(int irq, void *_pon)
{
	...
	u32 key_status = 0;
	uint pon_rt_sts = 0;
	char *work_state[2] = {"kpdpwr_work_state=0", NULL};
	...
	case PON_KPDPWR:
		...
		rc = devm_request_irq(pon->dev, cfg->state_irq,
				...
				IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING,
						"qpnp_kpdpwr_status", pon);
		*/
		/*End hankecai add kpdpwr state 2017-08-08*/
		if (rc < 0) {
...
static int qpnp_pon_probe(struct platform_device *pdev)
{
	...
	/*Begin hankecai kpdpwr state*/
	INIT_DELAYED_WORK(&pon->kpdpwr_work, kpdpwr_work_func);
	/*End hankecai kpdpwr state*/
	...
	rc = device_create_file(&pdev->dev, &dev_attr_kpdpwr_work_state);
	if (rc) {
		dev_err(&pdev->dev, "sys file kpdpwr_work_state creation failed\n");
		return rc;
	}
	rc = device_create_file(&pdev->dev, &dev_attr_kpdpwr_work_timeout);
	if (rc) {
		dev_err(&pdev->dev, "sys file work_timeout_secs creation failed\n");
		return rc;
	}

	/*End hankecai add set dloadmode to debug*/
	...
	if (rc) {
		dev_err(&pdev->dev, "sys file kpdpwr_volume_down_wake creation failed\n");
		return rc;
	}
	/*vivo sensor team add to enable disable volume dowm to wakeup*/
...
static int qpnp_pon_remove(struct platform_device *pdev)
{
	...
	device_remove_file(&pdev->dev, &dev_attr_kpdpwr_work_state);
	device_remove_file(&pdev->dev, &dev_attr_kpdpwr_work_timeout);
