...
static void gpio_keys_gpio_report_event(struct gpio_button_data *bdata)
{
	...
	if (type == EV_ABS) {
		if (state) {
			...
			printk(KERN_ERR "[HW_KEY] PMIC input: keycode=%d, status=0x%02X", button->code, button->value);
		}
	} else {
		input_event(input, type, *bdata->code, state);
		printk(KERN_ERR "[HW_KEY] PMIC input: keycode=%d, status=0x%02X", *bdata->code, state);
	}
	input_sync(input);
}
...
static void gpio_keys_gpio_work_func(struct work_struct *work)
{
	...
	if (bdata->button->code)
		printk(KERN_ERR "[HW_KEY] PMIC input: gpio_keys irq function: code=%d\n", bdata->button->code);
...
static int __maybe_unused gpio_keys_resume(struct device *dev)
{
	...
	for (i = 0; i < ddata->pdata->nbuttons; i++) {
		struct gpio_button_data *bdata = &ddata->data[i];
		state = gpiod_get_value_cansleep(bdata->gpiod);
		printk(KERN_ERR "gpio_keys_resume: keycode=%d, state=0x%hhx\n", bdata->button->code, state);
	}
	//gpio_keys_report_state(ddata);
	/* vivo sensor team end */
